...
class SecuritySupportSpec extends Specification {
    ...
    def "strips data from saved keys on read"() {
        ...
        then:
        keyrings.size() == 2
        keyrings.forEach { key ->
            assert key.publicKey.getUserAttributes().size() == 0
            assert key.publicKey.getSignatures().size() == 0
            assert key.publicKey.keySignatures.size() == 0
            assert key.publicKey.userIDs.size() == 0
        }
    }
