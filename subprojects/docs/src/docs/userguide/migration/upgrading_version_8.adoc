...
classpath. To avoid these conflicts, this behavior is deprecated and will be removed in Gradle 9.0.
...
by the suite itself by automatically declaring the proper dependencies on the test runtime classpath.
...
This change in behavior can also be mitigated without migrating to test suites. In this case, you must
manually declare the proper dependencies on the test runtime classpath. If you are using JUnit 5, an explicit
test runtime dependency on `org.junit.platform:junit-platform-launcher` is required. If you are using JUnit 4,
an explicit test runtime dependency on `junit:junit` is required. Note, for JUnit 4, the test runtime dependency
is only required if `junit` is not already declared as a `testImplementation` dependency or if JUnit 3 is used.
